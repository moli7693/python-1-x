...
def fibonacci_recursive(num):
    if num <= 2:
        return ____【1】____
    return fibonacci_recursive(num - 1) + fibonacci_recursive(num - ____【2】____)
...
def fibonacci_traversive(num):
    ...
    for i in range(3, num + 1):         
        ...
        temp1 = ____【3】____   # temp1和temp2分别更新赋值
        temp2 = temp
    # 返回第num个元素的计算结果
    return temp2

# 用两种方式分别计算Fibonacci序列前10个元素的值
for i in range(1, ____【4】____):
    print("第%d个元素值为：%d, %d" %
...
def test_fibonacci(func, num):
    ...
    result = ____【5】____(num)      # 调用函数
    end = time.time()       # 记录函数调用结束时间
    # 返回计算结果和耗时
    return (result, end - ____【6】____)
